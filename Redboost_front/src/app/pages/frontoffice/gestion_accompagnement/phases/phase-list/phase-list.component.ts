// src/app/components/phase-list/phase-list.component.ts
...
export class PhaseListComponent implements OnInit, OnDestroy {
    ...
    this.subscriptions.add(
      this.route.queryParams.subscribe(params => {
        ...
        if (this.projectId != null) {
          this.webSocketService.initialize(this.projectId);
  ...
  private setupWebSocketSubscriptions(): void {
    ...
    this.subscriptions.add(
      this.webSocketService.phaseUpdates$.subscribe(update => {
        if (update && this.projectId) {
          if ('action' in update && update.action === 'delete') {
            this.phases = this.phases.filter(phase => phase.phaseId !== update.phaseId);
            delete this.phaseTasks[update.phaseId];
          } else {
            const phase = update as Phase;
            if (phase.projetId === this.projectId) {
              const index = this.phases.findIndex(p => p.phaseId === phase.phaseId);
              if (index >= 0) {
                this.phases[index] = phase; // Update existing phase
              } else {
                this.phases.push(phase); // Add new phase
              }
              this.phases.sort((a, b) => new Date(a.createdAt!).getTime() - new Date(b.createdAt!).getTime());
              this.loadTasksForPhase(phase.phaseId!);
            }
          }
    ...
    this.subscriptions.add(
      this.webSocketService.taskUpdates$.subscribe(update => {
        if (update && this.projectId) {
          if ('action' in update && update.action === 'delete') {
            // Remove task from all phaseTasks
            Object.keys(this.phaseTasks).forEach(phaseId => {
              this.phaseTasks[+phaseId] = this.phaseTasks[+phaseId].filter(task => task.taskId !== update.taskId);
            });
          } else {
            const task = update as Task;
            if (task.phase?.phaseId && this.phases.some(phase => phase.phaseId === task.phase.phaseId)) {
              const phaseId = task.phase.phaseId;
              const taskList = this.phaseTasks[phaseId] || [];
              const index = taskList.findIndex(t => t.taskId === task.taskId);
              if (index >= 0) {
                taskList[index] = task; // Update existing task
              } else {
                taskList.push(task); // Add new task
              }
              this.phaseTasks[phaseId] = [...taskList]; // Trigger change detection
            }
          }
          this.calculateMetrics(); // Recalculate XP metrics
        }
      })
    );
  }
  ...
  loadPhases(): void {
    ...
    this.phaseService.getAllPhases().subscribe({
      next: data => {
        this.phases = data
          .filter(phase => !this.projectId || phase.projetId === this.projectId)
      ...
      error: err => {
        ...
        this.snackBar.open('Erreur lors du chargement des phases', 'Fermer', { duration: 3000 });
      }
    });
  }
  ...
  loadTasksForPhase(phaseId: number): void {
    this.taskService.getTasksByPhaseId(phaseId).subscribe({
      next: tasks => {
        this.phaseTasks[phaseId] = tasks;
      },
      error: err => {
        console.error(`Impossible de charger les tâches pour la phase ${phaseId} :`, err);
      }
    });
  }
  ...
  openEditDialog(phase: Phase): void {
    ...
    dialogRef.afterClosed().subscribe({
      ...
      error: err => {
        ...
        this.snackBar.open('Erreur lors de la mise à jour de la phase', 'Fermer', { duration: 3000 });
      }
    });
  }
  ...
  openCreateDialog(): void {
    ...
    dialogRef.afterClosed().subscribe({
      ...
      error: err => {
        ...
        this.snackBar.open('Erreur lors de la création de la phase', 'Fermer', { duration: 3000 });
      }
    });
  }
    ...
    this.evaluationService.getPendingEvaluationPhaseIds(this.currentUser.id).subscribe({
      next: (pendingPhaseIds: number[]) => {
        if (pendingPhaseIds && pendingPhaseIds.length > 0) {
          const phaseToEvaluate = this.phases.find(
            phase => phase.phaseId != null && phase.projetId === this.projectId && pendingPhaseIds.includes(phase.phaseId)
          );
          ...
          if (phaseToEvaluate) {
            ...
            this.openEvaluationFormDialog(
              phaseToEvaluate.phaseId!,
              phaseToEvaluate.phaseName,
              this.projectId!,
