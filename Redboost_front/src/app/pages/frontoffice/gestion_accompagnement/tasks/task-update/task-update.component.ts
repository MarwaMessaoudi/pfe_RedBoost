// src/app/components/task-update/task-update.component.ts
import { Component, OnInit, OnDestroy, ViewEncapsulation, Input, Output, EventEmitter, SimpleChanges, ChangeDetectorRef } from '@angular/core';
...
export class TaskUpdateComponent implements OnInit, OnDestroy {
  ...
  ngOnDestroy(): void {
    // No WebSocket subscription to clean up
  }
  ...
  startDateFilter = (date: Date | null): boolean => {
    ...
    return date >= earliestAllowed && (!validPhaseEnd || date <= validPhaseEnd);
  };
  ...
  fetchCurrentUser(): void {
    this.http.get('http://localhost:8085/users/profile').subscribe({
      ...
      error: error => {
        ...
        this.snackBar.open('Échec du chargement du profil utilisateur', 'Fermer', { duration: 3000 });
        this.statusOptions = Object.values(Status).filter(status => status !== Status.VALIDATED);
        this.cdr.detectChanges();
      }
    });
  ...
  loadTask(id: number): void {
    ...
    this.taskService.getTaskById(id).subscribe({
      ...
      error: error => {
        this.error = 'Échec du chargement de la tâche. Veuillez réessayer plus tard.';
        this.loading = false;
        this.snackBar.open('Échec du chargement des détails de la tâche', 'Fermer', { duration: 3000 });
        this.cdr.detectChanges();
      }
    });
  }
  ...
  loadAvailableCategories(): void {
    this.taskCategoryService.getAllTaskCategories().subscribe({
      ...
      error: err => {
        ...
        this.snackBar.open('Échec du chargement des catégories', 'Fermer', { duration: 3000 });
      }
    });
  }
  ...
  prepareTaskData(): Task {
    ...
    return {
      ...
      priority: formValues.priority || Priority.MEDIUM,
      status: formValues.status || Status.TO_DO,
  ...
  deleteTask(): void {
    ...
    this.taskService.deleteTask(this.task.taskId).subscribe({
      ...
      error: error => {
        this.loading = false;
        this.snackBar.open('Impossible de supprimer la tâche', 'Fermer', { duration: 3000 });
        this.cdr.detectChanges();
      }
    });
  }
  ...
  saveTitle(): void {
    if (!this.taskForm.value.title) {
      this.snackBar.open('Le titre ne peut pas être vide', 'Fermer', { duration: 3000 });
      return;
    }
    if (this.task) {
  ...
  saveXpPoint(): void {
    ...
    if (xpPointValue < 0) {
      this.snackBar.open('Les points XP ne peuvent pas être négatifs', 'Fermer', { duration: 3000 });
      return;
    }
    if (this.task) {
  ...
  saveTaskCategory(): void {
    if (this.task && this.selectedCategoryId !== undefined) {
      ...
      this.snackBar.open('Aucune catégorie sélectionnée', 'Fermer', { duration: 3000 });
      this.editingCategory = false;
    }
  }
  ...
  addComment(): void {
    ...
    if (!commentValue?.trim() || !this.task?.taskId || !this.currentUser) {
      this.snackBar.open('Le commentaire ne peut pas être vide ou l’utilisateur n’est pas chargé', 'Fermer', { duration: 3000 });
      return;
    }

    const newComment: Comment = {
      content: commentValue,
      profilePictureUrl: this.currentUser.profilePictureUrl, // Fixed typo
  ...
  trackByAttachments(index: number, attachment: Attachment): string {
    return attachment?.fileId || attachment?.name || `attachment-${index}`;
  }
  ...
  addSubTask(): void {
    if (this.newSubTask.title.trim()) {
      ...
      this.snackBar.open('Le titre de la sous-tâche est requis', 'Fermer', { duration: 3000 });
    }
  }
  ...
  fetchAttachmentName(fileId: string): void {
    this.http.get(`http://localhost:8085/api/drive/files/${fileId}/name`).subscribe({
      next: (response: any) => {
        this.attachmentName = response.name || 'Fichier inconnu';
      ...
      error: error => {
        console.error('Échec de la récupération du nom de la pièce jointe:', error);
        this.attachmentName = 'Fichier inconnu';
        if (this.task && this.task.attachment) {
          this.task.attachment.name = this.attachmentName;
        }
        this.snackBar.open('Impossible de récupérer le nom de la pièce jointe', 'Fermer', { duration: 3000 });
        this.cdr.detectChanges();
      }
    });
  }
  ...
  downloadAttachment(): void {
    if (!this.task?.taskId || !this.task.attachment?.fileId) {
      this.snackBar.open('Aucune pièce jointe disponible pour le téléchargement.', 'Fermer', { duration: 3000 });
      return;
    }
    this.taskService.downloadAttachment(this.task.taskId).subscribe({
      next: blob => {
        ...
        this.snackBar.open('Pièce jointe téléchargée avec succès.', 'Fermer', { duration: 3000 });
      },
      error: err => {
        console.error('Échec du téléchargement de la pièce jointe:', err);
        this.snackBar.open('Impossible de télécharger la pièce jointe.', 'Fermer', { duration: 3000 });
      }
    });
  }
